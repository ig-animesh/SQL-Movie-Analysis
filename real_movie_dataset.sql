...
-- 1.What are the top 5 highest rated movies based on ratings?
...
SELECT g.genre_name, COUNT(mg.movie_id) AS TotalMovies
FROM genres AS g
...
SELECT a.name, COUNT(ma.movie_id) AS TotalMovies
FROM actors AS a
...
SELECT u.name, ROUND(AVG(r.rating), 1) as AvgRating
FROM users AS u
JOIN ratings r ON
u.user_id = r.user_id
GROUP BY u.name;
...
RANK() OVER(ORDER BY release_year) AS Rankings
FROM movies;

-- 7.Classify movies as 'Classic', 'Mordern' or 'New' based on release year.
SELECT title, release_year,
CASE
     WHEN release_year < 2000 THEN 'Classic'
	 WHEN release_year > 2000 AND release_year < 2020 THEN 'Mordern'
	 ELSE 'New'
   END AS EraBuket
FROM movies;

-- 8.Recommended Movie of the day.
SELECT m.title AS TITLE, m.release_year AS YEAR, g.genre_name AS GENRE, a.name AS ACTOR, r.rating AS ratings
FROM movies AS m
JOIN movie_actors ma ON
